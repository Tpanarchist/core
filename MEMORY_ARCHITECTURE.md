...
| 2 | `sqlite_store` | `SqliteMemoryStore`, `StoreCorruption`, `UnsupportedSchemaVersion`, `SqliteStoreClosed` | `store`, `codec`, `episode`, `recall`, `retention`, `core.identity`, `core.time`, `core.context`, `core.value`, `core.epistemic`, `core.observation`, `core.event`, `core.effect`, `core.provenance`, `core.error`, stdlib `sqlite3`/`hashlib`/`hmac`/`os` *(frozen by Pass 3 preregistration, `docs/memory-passes/03-sqlite-backend.md` §79 — if implementation proves one of the Memory/Core edges unnecessary, omit it; no `memory.belief`/`core.state`/`core.trace`/`core.transform` edge is ever justified)* |
